...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/15 14:58:32 by scuter            #+#    #+#             */
/*   Updated: 2021/08/24 18:59:48 by scuter           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buff)
		return (NULL);
	while (!has_newline(stack[fd]) && ret != 0)
	{
		...
		buff[ret] = '\0';
		stack[fd] = str_join(stack[fd], buff);
	}
	free(buff);
	return (ret_line(&stack[fd]));
}
